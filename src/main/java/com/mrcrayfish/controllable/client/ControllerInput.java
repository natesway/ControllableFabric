...
public class ControllerInput
{
            ...
            if(Math.abs(this.mouseSpeedX) > 0F || Math.abs(this.mouseSpeedY) > 0F)
            {
                ...
                if(hoveredListener instanceof AbstractSelectionList<?> list)
                {
                    ...
                    for(int i = 0; i < count; i++)
                    {
                        ...
                        if(rowTop < list.y0 && rowBottom > list.y1) // Is visible
                            continue;
    ...
    @SubscribeEvent
    public void onScreenInit(ScreenEvent event)
    {
        Minecraft mc = Minecraft.getInstance();
        if(mc.screen == null)
        {
    ...
    @SubscribeEvent
    public void onRenderScreen(ScreenRenderBeforeEvent event)
    {
                ...
                if(mc.getOverlay() == null)
                {
                    ...
                    if(mc.mouseHandler.activeButton != -1 && mc.mouseHandler.lastMouseEventTime > 0.0D)
                    {
                        Screen.wrapScreenError(() ->
                        {
                            ...
                            if(/*net.minecraftforge.client.ForgeHooksClient.onScreenMouseDragPre(screen, finalMouseX, finalMouseY, mc.mouseHandler.activeButton, finalDragX, finalDragY)*/false)
                            {
                                return;
                            }
    ...
    public void onRender(TickEvent.ClientTickEvent event)
    {
        ...
        if(mc.screen == null)
        {
            ...
            if(Math.abs(controller.getRThumbStickXValue()) >= deadZone)
            {
                ...
                if(!/*MinecraftForge.EVENT_BUS.post(turnEvent)*/false)
                {
    ...
    public void onOpenScreen(ScreenEvent event)
    {
        ...
        if(Config.SERVER.restrictToController.get() && mc.level != null && !this.isControllerInUse())
        {
            ...
            {
                //event.setCanceled(true); //TODO: Implement cancellation in thonkutil
            }
        }
    }

    @SubscribeEvent
    public void onMouseClicked(RawMouseClickEvent event)
    {
        Minecraft mc = Minecraft.getInstance();
    ...
    public void onInputUpdate(MovementInputUpdateEvent event)
    {
        ...
        if(mc.screen == null)
        {
            if((!RadialMenuHandler.instance().isVisible() || Config.CLIENT.options.radialThumbstick.get() != Thumbstick.LEFT) && !/*MinecraftForge.EVENT_BUS.post(new ControllerEvent.Move(controller))*/false)
            {
                float deadZone = Config.CLIENT.options.deadZone.get().floatValue();
    ...
    public void handleButtonInput(Controller controller, int button, boolean state, boolean virtual)
    {
        ...
        if(button != -1)
        {
            ...
            if(/*MinecraftForge.EVENT_BUS.post(eventInput)*/false)
                return;
    ...
    private void scrollCreativeTabs(CreativeModeInventoryScreen screen, int dir)
    {
        ...
        if(dir > 0)
        {
            ...
            {
                screen.selectTab(CreativeModeTab.TABS[screen.getSelectedTab() + 1]);
            }
        }
        else if(dir < 0)
        {
    ...
    private List<NavigationPoint> gatherNavigationPoints(Screen screen)
    {
        ...
        if(screen instanceof AbstractContainerScreen<?> containerScreen)
        {
            ...
            {
                if(containerScreen.hoveredSlot == slot)
                    continue;
        ...
        {
            int tabPage = ((CreativeGuiExtensions)screen).fabric_currentPage();
    ...
    private BasicNavigationPoint getCreativeTabPoint(AbstractContainerScreen<?> screen, CreativeModeTab tab)
    {
        ...
        int width = 28;
        int height = 32;
        int x = screen.leftPos + width * column;
        int y = screen.topPos;
        x = tab.isAlignedRight() ? screen.leftPos + screen.imageWidth - width * (6 - column) : (column > 0 ? x + column : x);
        y = topRow ? y - width : y + (screen.imageHeight - 4);
    ...
    private void craftRecipeBookItem()
    {
        ...
        if(recipeButton != null)
        {
            ...
            {
                this.invokeMouseClick(screen, GLFW.GLFW_MOUSE_BUTTON_LEFT, screen.leftPos + slot.x + 8, screen.topPos + slot.y + 8);
            }
            else
            {
                this.invokeMouseReleased(screen, GLFW.GLFW_MOUSE_BUTTON_LEFT, screen.leftPos + slot.x + 8, screen.topPos + slot.y + 8);
            }
        }
    }
    ...
    private void invokeMouseClick(Screen screen, int button, double mouseX, double mouseY)
    {
        ...
        if(screen != null)
        {
            ...
            Screen.wrapScreenError(() ->
            {
                boolean cancelled = /*ForgeHooksClient.onScreenMouseClickedPre(screen, mouseX, mouseY, button);*/false;
                if(!cancelled)
                {
    ...
    private void invokeMouseReleased(Screen screen, int button, double mouseX, double mouseY)
    {
        ...
        if(screen != null)
        {
            ...
            Screen.wrapScreenError(() ->
            {
                boolean cancelled = /*ForgeHooksClient.onScreenMouseReleasedPre(screen, mouseX, mouseY, button);*/false;
                if(!cancelled)
                {
