...
public class ControllerToggleSetting extends ControllerSetting<Boolean>
{
    ...
    {
        return () -> {var q = CycleButton.onOffBuilder(this.configValue.get()).withTooltip(value -> this.tooltip).create(x, y, width, height, this.label, (button, value) -> {
            this.configValue.set(value);
            Config.save();
        });if(this.disabled)q.active=false;return q;};
    }
