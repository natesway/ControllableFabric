...
public class ControllerEnumSetting<T extends Enum<?> & SettingEnum> extends ControllerSetting<T>
{
    ...
    {
        return () -> CycleButton.builder(T::getLabel).withValues(this.values).withInitialValue(this.configValue.get()).withTooltip(value -> this.tooltip).create(x, y, width, height, this.label, (button, value) -> {
